-- If LuaRocks is installed, make sure that packages installed hrough it are
-- found (e.g. lgi). If LuaRocks is not installed, do nothing.

--Install brightnessctl, acpi, lxqt-powermanager
...
-- {{{ Autostart
--Run -ll the apps listed in configuration/apps.lua as run_on_start_up only once when awesome start
local apps = {
...
awful.screen.connect_for_each_screen(function(s)
	...
	s.mywibox:setup({
		...
		{ -- Right widgets
			layout = wibox.layout.fixed.horizontal,
			awful.widget.watch("bash -c \"sensors | grep 'temp1' | awk '{print $2}'\" ", 3),
			wibox.widget.textbox("  |  "),
			awful.widget.watch("acpi -b", 10, function(widget, stdout)
				-- Extract just the percentage number
				local charge = stdout:match("(%d?%d?%d)%%")
				if charge then
					widget.text = charge .. "%"
				else
					widget.text = "N/A"
...
globalkeys = gears.table.join(
	awful.key({ "Control" }, "¿", function()
		awful.util.spawn("amixer -q -D pulse sset Master 2%+")
	...
	awful.key({}, "XF86MonBrightnessUp", function()
		awful.spawn.with_shell("brightnessctl set +200")
...
awful.rules.rules = {
	...
	{
		rule_any = { class = { "firefox", "libreoffice" } },
		properties = { tag = "2" },
	},
	{
		rule_any = { class = { "Apache NetBeans IDE 27" } },
		properties = { tag = "3" },
	},
	{
